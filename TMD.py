...
class OffPolicyPPOAgent:
    def __init__(self, input_dim, hidden_dim, num_layers, action_dim, lr=0.001, epsilon=0.2, gamma=0.99, beta=0.01):
        """
        Off-Policy PPO Agent using TLSTM for state processing and MLP for policy and value estimation.
        ...
        :param epsilon: Clipping range for PPO updates.
...
class TransportationModeDetector:
    def __init__(self, input_dim, hidden_dim, num_layers, attention=True):
...
if __name__ == "__main__":
    ...
    detector = TransportationModeDetector(input_dim=X_train.shape[1], hidden_dim=50, num_layers=3, attention=True)
